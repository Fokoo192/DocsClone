import 'package:docs_clone/colors.dart';
import 'package:docs_clone/repository/auth_repo.dart';
import 'package:docs_clone/screens/home_screen.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:routemaster/routemaster.dart';

class LoginScreen extends ConsumerWidget {
  const LoginScreen({super.key});

  void signInWithGoogle(BuildContext context, WidgetRef ref) async {
    final sMessenger = ScaffoldMessenger.of(context);
    final navigator = Routemaster.of(context); //returns an instance of navigator from context of the widget
    final errorModel = await ref.read(authRepoProvider).signInWithGoogle();
    if (errorModel.error == null) {
      //no error
      ref
          .read(userProvider.notifier)
          .update((state) => errorModel.data); //notifer is used to make changes
      navigator.replace("/");
    } else {
      sMessenger.showSnackBar(
        SnackBar(
          content: Text(errorModel.error!),
        ),
      );
    }
  }

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return Scaffold(
      body: Center(
        child: ElevatedButton.icon(
          onPressed: () => signInWithGoogle(context, ref),
          icon: Image.asset(
            "assets/images/g-logo-2.png",
            height: 20,
          ),
          label: const Text(
            "Sign in with Google",
            style: TextStyle(color: ColorBlack),
          ),
          style: ElevatedButton.styleFrom(
              minimumSize: const Size(150, 50), backgroundColor: ColorWhite),
        ),
      ),
    );
  }
}
